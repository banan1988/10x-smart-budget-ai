...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (user) {
    ...
    if (shouldFetchProfile) {
      ...
      console.log('[Middleware] Profile fetch duration:', { profileDuration, pathname: url.pathname, role: locals.user.role });
    }
  ...
  if (isAdminPath && !url.pathname.startsWith('/api/')) {
    ...
    if (user && locals.user?.role !== 'admin') {
      console.log('[Middleware] Non-admin user blocked from admin page:', { pathname: url.pathname, role: locals.user?.role });
