...
describe("useLoginForm", () => {
  ...
  describe("Email validation", () => {
    ...
    it("should accept valid email format", () => {
      ...
      // Assert
      expect(result.current.state.emailError).toBeUndefined("Should not show error for valid email");
    });
    ...
    it("should show error for empty email", () => {
      ...
      // Assert
      expect(result.current.state.emailError).toBe("Email jest wymagany", "Should show required email error");
    });
    ...
    it("should clear email error on change", () => {
      ...
      // Assert
      expect(result.current.state.emailError).toBeUndefined("Should clear email error when valid email is provided");
    });
  });
  ...
  describe("Password validation", () => {
    ...
    it("should accept valid password", () => {
      ...
      // Assert
      expect(result.current.state.passwordError).toBeUndefined("Should not show error for valid password");
    });
    ...
    it("should show error for empty password", () => {
      ...
      // Assert
      expect(result.current.state.passwordError).toBe("Hasło jest wymagane", "Should show required password error");
    });
  ...
  describe("Form validation", () => {
    ...
    it("should return isFormValid = false when password is invalid", () => {
      ...
      // Assert
      expect(result.current.isFormValid).toBe(false, "Form should be invalid when password is invalid");
    });
  ...
  describe("Form submission", () => {
    ...
    it("should set loading state during submission", async () => {
      ...
      // Assert
      await waitFor(() => {
        expect(result.current.state.isLoading).toBe(false, "Loading state should be false after successful submission");
      });
    });
    ...
    it("should handle invalid credentials error (401)", async () => {
      ...
      // Assert
      expect(result.current.state.passwordError).toBeTruthy("Password error should be set for invalid credentials");
      expect(result.current.state.password).toBe("", "Password should be cleared on failed login");
      expect(result.current.state.isLoading).toBe(false, "Loading state should be false after error");
    ...
    it("should handle email not confirmed error (403)", async () => {
      ...
      // Assert
      expect(result.current.state.generalError).toBeTruthy("General error should be set for email not confirmed");
      expect(toast.error, "Toast error should be shown").toHaveBeenCalled();
    });
    ...
    it("should handle too many attempts error (429)", async () => {
      ...
      expect(toast.error, "Toast should show rate limit error message").toHaveBeenCalledWith(expectedErrorMessage);
    });
    ...
    it("should handle network error", async () => {
      ...
      // Assert
      expect(result.current.state.generalError).toBe("Network error", "Should display network error message");
      expect(result.current.state.isLoading).toBe(false, "Loading state should be false after network error");
      expect(toast.error, "Toast should show network error").toHaveBeenCalledWith("Network error");
    });
  });
  ...
  describe("Touched state", () => {
    it("should mark field as touched on blur", () => {
      ...
      // Assert
      expect(result.current.state.touched.email).toBe(true, "Email field should be marked as touched");
      expect(result.current.state.touched.password).toBe(false, "Password field should not be touched");
    });
    ...
    it("should mark all fields as touched on submit attempt", async () => {
      ...
      // Assert
      expect(result.current.state.touched.email).toBe(true, "Email field should be marked as touched after submit");
