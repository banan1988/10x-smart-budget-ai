...
describe('useDashboardStats', () => {
  ...
  it('should fetch and map dashboard stats correctly', async () => {
    ...
    expect(result.current.data?.categoryBreakdown).toHaveLength(
      2,
      'Should map all categories from backend'
    );
  ...
  it('should handle fetch error', async () => {
    ...
    expect(result.current.error?.message).toContain('Failed to fetch stats', 'Error message should contain specific text');
  });
  ...
  it('should map all categories from backend', async () => {
    ...
    expect(result.current.data?.categoryBreakdown).toHaveLength(
      7,
      'Should map all categories without limiting'
    );
    expect(result.current.data?.categoryBreakdown[0].name).toBe(
      'Cat1',
      'First category should be mapped correctly'
    );
    expect(result.current.data?.categoryBreakdown[6].name).toBe(
      'Cat7',
      'Last category should be mapped correctly'
    );
  });
  ...
  it('should handle negative balance correctly', async () => {
    ...
    expect(balanceMetric?.variant).toBe(
      'balance-negative',
      'Should use balance-negative variant for negative balance'
    );
    expect(balanceMetric?.value).not.toContain(
      '+',
      'Negative balance should not have + sign'
    );
  });
  ...
  it('should handle network errors gracefully', async () => {
    ...
    expect(result.current.error?.message).toContain(
      ...
      'Should preserve network error message'
    );
  });
  ...
  it('should handle empty category breakdown', async () => {
    ...
    expect(result.current.data?.categoryBreakdown).toHaveLength(
      0,
      'Should handle empty category breakdown'
    );
  ...
  it('should handle empty daily breakdown', async () => {
    ...
    expect(result.current.data?.dailyBreakdown).toHaveLength(
      0,
      'Should handle empty daily breakdown'
    );
