...
describe("useAdminFeedbacks hook", () => {
  ...
  it("should fetch feedbacks on mount", async () => {
    ...
    expect(result.current.feedbacks).toEqual(mockData.data, "Should load feedbacks from API response");
  ...
  it("should initialize with provided filters", () => {
    ...
    expect(result.current.filters).toEqual(initialFilters, "Should initialize with provided filters");
  ...
  it("should handle page boundary validation", async () => {
    ...
    expect(result.current.page).toBe(1, "Should not allow page less than 1");
  ...
  it("should handle network error gracefully", async () => {
    ...
    expect(result.current.isLoading).toBe(false, "Should stop loading on network error");
  });
  ...
  it("should handle non-JSON response gracefully", async () => {
    ...
    expect(result.current.feedbacks).toEqual([], "Should keep feedbacks empty for non-JSON response");
  });
  ...
  it("should refetch data when calling refetch()", async () => {
    ...
    expect(result.current.feedbacks.length).toBe(1, "Should load initial data with 1 feedback");
    ...
    expect(result.current.feedbacks.length).toBe(2, "Should update data after refetch with 2 feedbacks");
    expect(result.current.totalCount).toBe(2, "Should update total count after refetch");
  ...
  it("should not refetch if page value does not change", async () => {
    ...
    expect(fetchSpy.mock.calls.length).toBe(initialCallCount, "Should not fetch again when page does not change");
  });
  ...
  it("should handle 401 unauthorized error", async () => {
    ...
    expect(result.current.error).toBe("Unauthorized access", "Error message should contain the API error message");
  });
});
