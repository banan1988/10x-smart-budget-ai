  ...
  describe("createFeedback", () => {
    it("should create feedback and return FeedbackDto with all fields", async () => {
      ...
      mockSupabase.from = vi.fn(() => ({
        insert: vi.fn(() => ({
          select: vi.fn(() => ({
            single: vi.fn(() => Promise.resolve({ data: feedbackResponse, error: null })),
          })),
        })),
      }));

      // Act
      const result = await FeedbackService.createFeedback(mockSupabase, mockUserId, mockFeedbackData);
    ...
    it("should throw error when database fails during insert", async () => {
      ...
      // Act & Assert
      await expect(FeedbackService.createFeedback(mockSupabase, mockUserId, mockFeedbackData)).rejects.toThrow(
        "Failed to create feedback: Database constraint violation"
      );
    });
    ...
    it("should throw error when no data returned despite no error", async () => {
      ...
      // Act & Assert
      await expect(FeedbackService.createFeedback(mockSupabase, mockUserId, mockFeedbackData)).rejects.toThrow(
        "Failed to create feedback: No data returned"
      );
    });
