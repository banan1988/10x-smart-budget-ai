      ...
      vi.mocked(createSupabaseServerInstance).mockReturnValue({
        auth: {
          signUp: vi.fn().mockResolvedValue(
            mockSupabaseAuthSuccess('user-456', 'newuser@example.com')
          ),
        },
      } as any);
  ...
  describe('Invalid registration', () => {
    it('should return 409 when email already exists', async () => {
      ...
      vi.mocked(createSupabaseServerInstance).mockReturnValue({
        auth: {
          signUp: vi.fn().mockResolvedValue(
            mockSupabaseAuthError('user_already_exists', 'User already registered', 409)
          ),
        },
      } as any);
  ...
  describe('Error handling', () => {
    it('should return 500 on Supabase unexpected error', async () => {
      ...
      vi.mocked(createSupabaseServerInstance).mockReturnValue({
        auth: {
          signUp: vi.fn().mockResolvedValue(
            mockSupabaseAuthError('unknown_error', 'Unknown error', 500)
          ),
        },
      } as any);
